...
class Guren extends Frontend
{
    ...
    public function lists() {
        ...
        $page = (int)$page;
        $pageSize = 40;
        $path = "/juzi/Guren/lists/";
        $whereSql = [
            'typeid' => juziFromModel::$authorTypeGuren
        ];
        if($keyword) {
    ...
    public function id() {
        ...
        $path_ = $request->path();
        if(!strstr($path_, '/Guren/id/')) {
            print_r('路径不正确,必须是/Guren/id/');
            exit;
        }
        $array_ = explode('/Guren/id/', $path_);
        $endStr = $array_[1];
        $endStr = trim($endStr);
