...
class Auth
{
    ...
    {
        return $this->_user ? ($this->_user->$name = $value) : NULL;
    }
    ...
    public function init($token)
    {
        ...
        if ($user_id > 0)
        {
            ...
            $this->_logined = TRUE;
            $this->_token = $token;
            $this->_third = $data['third'];
            $this->_extra = (array)$data['extra'];
     ...
     * @param bool $checkusername    是否检测username
     * @return boolean
     */
    public function register($username, $password, $email = '', $mobile = '', $extend = [], $checkusername = true)
    {
        // 检测用户名或邮箱、手机号是否存在
        if ($checkusername && $username && Users::getByUsername($username))
        {
        ...
        try
        {
            ...
            if(isset($params['invite_code']) && $params['invite_code']) {
                ...
                if($inviteAddon && method_exists($inviteAddon, 'runInviteKeyFunc')) {
                    $inviteAddon->runInviteKeyFunc($params['invite_code'], $user->id, 'reg');
                }
            }
            Db::commit();
            ...
            $this->_token = Token::set($this->_token, $user->id, $this->keeptime);
     ...
     * @return boolean
     */
    public function moblie_register($mobile = '', $extend = [])
    {
        // 检测用户名或邮箱、手机号是否存在
        if ($mobile && Users::getByMobile($mobile))
        {
        ...
        $data = [
            'username' => $mobile,  //当用手机注册时，手机为默认昵称
            'mobile'   => $mobile,
            'level'    => 1,
            'score'    => 0,
            'avatar'   => '',
     ...
     * @return boolean
     */
    public function login($account, $password, $where = [], $extra = [])
    {
        $field = Validate::is($account, 'email') ? 'email' : (Validate::is($account, 'mobile') ? 'mobile' : 'username');

        $user = Users::get([$field => $account] + $where);
        if (!$user)
        {
     ...
     * @return boolean
     */
    public function direct($user_id, $third = 0, $extra = [])
    {
        $user = Users::get($user_id);
        if ($user)
        {
			...
			$this->_third = $third;
			$this->_extra = $extra;
            $this->_token = Token::set($this->_token, $user->id, $this->keeptime, $third, $extra);
    ...
    public function getUserinfo()
    {
		...
		if(isset($userinfo['avatar']))$userinfo['avatar'] = long_url($userinfo['avatar']);
        $userinfo['token'] = $this->getToken();
		
		$third = \fast\Addon::getModel('usercenter', 'Third');
		$thirdinfo = [];
		$thirdlist = [];
		if($this->_third){
			$thirdinfo = $third->get($this->_third)->toArray();
		}
		if(!empty($this->id)){
            $thirdlist = (array)$third->where(['user_id' => $this->id])->select();
        }
        return ['userinfo' => $userinfo, 'thirdinfo' => $thirdinfo, 'thirdlist' => $thirdlist, 'extra' => $this->_extra];
    }
