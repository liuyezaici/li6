...
//            exit;
    ...
    public function clone($ids=NULL) {
        $ids = intval($ids);
        if(!$ids) $this->error('no ids');
        $info = AuthGroup::get($ids);
        $info = json_decode(json_encode($info), true);
        unset($info['id']);
        $info['name'] .= '(copy)';
        AuthGroup::insert($info);
...
//            print_r($currentRuleIds);exit;

            if (!$id || !in_array($pid, $this->childrenGroupIds) || $pid==1|| !in_array($pid, $groupTree->getChildrenIds($id, true))) {
                ...
                $this->error(__('Can not change the parent to child:'.$pid));
            }
        } else {
    ...
    public function members($ids=NULL)
    {
        if($this->request->isPost()) {
            ...
            if($email) {
                $uid = Admin::getfieldbyemail($email, 'id');
                $where_['id'] = $uid;
            }
            $total = Admin::where($where_)->count();
            $list = Admin::field('id,email')->where($where_)
                ->order('id', 'desc')
                ->page($page, $pageSize)
                ->select();
            unset($v);
