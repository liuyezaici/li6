...
use app\admin\addon\setting\model\Setting as SettingModel;
...
class Index extends Backend
{
    public function _initialize()
    {
        ...
        $this->view->assign('valTypeWords', SettingModel::$valTypeWords);
        $this->view->assign('valTypeText', SettingModel::$valTypeText);
        $this->view->assign('valTypePic', SettingModel::$valTypePicture);
        $this->view->assign('valTypeJson', SettingModel::$valTypeJson);
        $this->view->assign('allValType', json_encode(SettingModel::getValAllTypesForRadio()));
    }
...
//            print_r($postData);exit;
            ...
            $postData['ctime'] = time();
            SettingModel::insert($postData);

            $newSid = SettingModel::getLastInsID();
            //附件更新sid
            if($newSid && ($value && SettingModel::valTypeIsPic($postData['keytype']))) {
...
//                exit;
            ...
            if ($params)
            {
                ...
                //附件更新sid
                if($value && SettingModel::valTypeIsPic($params['keytype'])) {
                    $fujianModel = Addon::getModel('fujian');
                    if(!$fujianModel) $this->error('未安装fujian组件');
                    $fujianModel->updateSid($id, [$value]);
                }
            }
...
//            print_r(json_encode($where));exit;
            ...
            $total = SettingModel::where($where)->count();
            $list = SettingModel::where($where)
                ->order($sort, $order)
                ->page($page, $pageSize)
                ->select();

            foreach ($list as $n =>&$v) {
                $v['typenames'] =  SettingTypeModel::getTitles($v['typeid'], '<br />');
                $v['user_name'] =  Users::getfieldbyid($v['cuid'], 'username');
                $v['value'] =  Str::nohtml($v['value']);
                unset($v);
            }
