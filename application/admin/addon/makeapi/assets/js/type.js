define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'adminlte'], function ($, undefined, Backend, Table, Form, Adminlte) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'addon/makeapi/type/index',
                    add_url: 'addon/makeapi/type/add',
                    edit_url: 'addon/makeapi/type/edit',
                    del_url: 'addon/makeapi/type/del',
                }
            });
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        {field: 'id', title: 'ID'},
                        {field: 'title', title:'类型名'},
                        {field: 'keyname', title:'索引名'},
                        {field: 'ext_field', title: '扩展字段', formatter: function (value, row, index) {
                            var backArray = [];

                            var obj_ = row.ext_field;
                            $.each(obj_, function (item_, val_) {
                                backArray.push('【<span style="margin: 12px 0;">'+val_.keyname+'　'+val_.title+'　'+val_.type_name+'</span>】');
                            });
