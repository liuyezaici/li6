...
namespace app\admin\addon\fujian\model;
...
class Fujian extends Model
{
    ...
    public static function saveFile($uid, $fileInfo, $pubPath, $addonName, $addonSourceId, $fromTmp=true, $newName='', $isImg=true) {
        ...
        if($addonSourceId) {
            ...
            if($addonClass) {
                ...
                if( method_exists($addonClass, 'refreshTotalPics')) {
                    $addonClass->refreshTotalPics($addonSourceId);
                }
            }
        }
        $fileData = [
    ...
    public static function removeAddonFile($addonName, $addonSourceId) {
        ...
        //更新原数据的附件索引
        $addonClass = Addon::getModel($addonName);
        if($addonClass && method_exists($addonClass, 'refreshTotalPics')) {
            $addonClass->refreshTotalPics($addonSourceId);
        }
