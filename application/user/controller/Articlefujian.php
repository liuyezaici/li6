...
class Articlefujian extends Backend
{
    ...
    final function upload_files() {
        ...
        $rightHash = \fast\Str::makeSafeUploadCode($savePath, $userId);
        if($pathSafeHash != $rightHash ) {
            $this->error('文件上传目录被手动篡改了! '. $pathSafeHash . '|'. $rightHash);
        }
        //获取分享信息
        $articleInfo = Article::get($a_id);
        if(!$articleInfo) {
            $this->error('文章不存在');
        }
        $cuid = $articleInfo['cuid'];
        if($cuid != $userId) {
            $this->error('身份已经切换');
        }
        if (!empty($fileData)) {
            ...
            //允许的文件后缀
            //得到文件原名
            $fileNewName = \fast\Str::getRandChar(16). '.'. $geshi;
            $parthUrl = '/'.trim($savePath, "/") . '/'.  $fileNewName;
            $saveRoot = ROOT_PATH. trim($savePath, "/");
            File::creatdir($saveRoot);
            $targetUrl = $saveRoot. '/'. $fileNewName;
            if (move_uploaded_file($tempFile, $targetUrl)){
                fileModel::insert([
                    'sid' => $a_id,
                    'adduid' => $cuid,
                    'addtime' => \fast\Date::toYMDS(time()),
                    'filename' => $fileName,
                    'fileurl' => $parthUrl,
                    'filesize' => $fileSize,
                    'geshi' => $geshi,
                    'status' => 0,
                    'order' => fileModel::getLastOrder($a_id) + 10,
                ]); //写入文件索引
                $this->success('上传成功');
            }else{
                $this->error('上传失败');
    ...
    public function move_post_fujian() {
        ...
        $fileInfo = fileModel::getFileById($id);
        if(!$fileInfo) {
            $this->error('文件不存在');
        }
        $pid = $fileInfo['sid'];
        ...
        if($direction == 'l') {
            ...
            fileModel::editFile($id, ['order'=> $leftFileInfo['order'] + 1]);
            fileModel::editFile($leftFileInfo['id'], ['order'=> $leftFileInfo['order']-1]);
        } else {
            //往后面
            $rightFileInfo = fileModel::getPostFileRight($pid, $order, "id,order");
            if(!$rightFileInfo) $this->error('最右边了');
            fileModel::editFile($id, ['order'=> $rightFileInfo['order']-1]);
            fileModel::editFile($rightFileInfo['id'], ['order'=> $rightFileInfo['order'] +1]);
        }
        $this->success('修改成功');
    }
    //加载当前分享的附件
    public function load_article_fujians() {
        ...
        $pagesize = 10;
        $result = fileModel::where($where)->order('order', 'Desc')->paginate($pagesize, false,
            [
                'page' => $page,
            ]
        );
