...
class Sanwen extends Frontend
{
    ...
    public function article() {
        ...
        $path_ = $request->path();
        if(!strstr($path_, '/sanwen/article/')) {
            print_r('路径不正确,必须是/sanwen/article/,当前:'.$path_);
            exit;
        }
        $array_ = explode('/sanwen/article/', $path_);
        $endStr = $array_[1];
        $endStr = trim($endStr);
    ...
    public function main() {
        ...
        $request = Request::instance();
        $path_ = $request->path();
        if(!strstr($path_, '/sanwen/main/')) {
            print_r('路径不正确,必须是/sanwen/main/,当前:'.$path_);
            exit;
        }
        $array_ = explode('/sanwen/main/', $path_);
        $endStr = $array_[1];
        $endStr = trim($endStr);
        $fromId = trim($endStr, '/');
        ...
        $this->view->assign('webTitle', '散文:'.$fromName.'的文章['.  self::$webTitle.']');
        $this->view->assign('webdesc', self::$webdesc);//站点名
        $this->view->assign('fromName', $fromName);
        $this->view->assign('rq', $rq);
        $this->view->assign('authorStr', $authorid ? "<a href=\"/juzi/author/id/{$authorid}\" target='_blank'>{$authorName}</a>":'');
    ...
    public function index() {
        ...
        $query= [];
        $whereSql = [
            'fromtype'=>juziFromModel::$fromtypeSanwen
        ];
        if($authorid) {
...
//        $allYear = yearModel::select();
        // 获取分页显示
        $pageMenu = $list->render();
        $this->view->assign('webTitle', $filterTitle.'散文['.  self::$webTitle.']');
        $this->view->assign('webdesc', self::$webdesc);//站点名
        $this->view->assign('nullStr', $nullStr);
