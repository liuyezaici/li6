(function(global__) {
    ...
    (function(D) {
        function p(b, e, c) {
            ...
            if ("HEX" === e) { if (0 !== b.length % 2) throw "srcString of HEX type must be in byte increments";
                g = v(b);
                a = g.binLen;
                d = g.value } else if ("TEXT" === e) g = w(b, f), a = g.binLen, d = g.value;
        ...
        function y(b) { var e = [],
            c, a; for (a = 0; a < b.length; a += 1) c = b.charCodeAt(a), (a >>> 2) + 1 > e.length && e.push(0), e[a >>> 2] |= c << 24 - a % 4 * 8; return { value: e, binLen: 8 * b.length } }
        ...
        function z(b, e) { var c = "",
            a = 4 * b.length,
            d, f; for (d = 0; d < a; d += 1) f = b[d >>> 2] >>> 8 * (3 - d % 4), c += "0123456789abcdef".charAt(f >>> 4 & 15) + "0123456789abcdef".charAt(f & 15); return e.outputUpper ? c.toUpperCase() : c }
        ...
        function B(b) { var e = "",
            c = 4 * b.length,
            a, d; for (a = 0; a < c; a += 1) d = b[a >>> 2] >>> 8 * (3 - a % 4) & 255, e += String.fromCharCode(d); return e }
    ...
    global__.wapPubFunc.tokenPost({
        ...
        err_func: function (res) {
            if(wapPubFunc.isLogOut(res.code)) {
                wapPubFunc.getWeixinOpenid();
                return;
            }
            msgTisf('扫描初始化异常：'+ res.msg);
        }
    });
