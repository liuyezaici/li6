...
class XDB_R
{
	...
	var $hash_prime = 0;
	private $_io_times = 1;
	private $XDB_VERSION = 34;// 0x01 ~ 0xff
	private $XDB_TAGNAME = 'XDB';// First bytes
	private $XDB_MAXKLEN = 0xf0; // maxklen: < 255
	...
	function Get($key)
	{
		...
		$klen = strlen($key);
		if ($klen == 0 || $klen > $this->XDB_MAXKLEN)
		return false;
	...
	function _check_header($fd)
	{
		...
		if ($hdr['tag'] != $this->XDB_TAGNAME) return false;
