...
{	
    ...
    /** defines for ruleset */
    private $PSCWS4_RULE_MAX = 31;	// just 31, PHP do not support unsigined Int
    private $PSCWS4_RULE_SPECIAL = 0x80000000;
    private $PSCWS4_RULE_NOSTATS = 0x40000000;
    private $PSCWS4_ZRULE_NONE = 0x00;
    private $PSCWS4_ZRULE_PREFIX = 0x01;
    private $PSCWS4_ZRULE_SUFFIX = 0x02;
    private $PSCWS4_ZRULE_INCLUDE = 0x04;// with include
    private $PSCWS4_ZRULE_EXCLUDE = 0x08;// with exclude
    private $PSCWS4_ZRULE_RANGE = 0x10;	// with znum range

    /** defines for mode of scws <= 0x800 */
    private $PSCWS4_IGN_SYMBOL = 0x01;
    private $PSCWS4_DEBUG = 0x02;
    private $PSCWS4_DUALITY = 0x04;

    /** multi segment policy >= 0x1000 */
    private $PSCWS4_MULTI_NONE = 0x0000;	//  nothing
    private $PSCWS4_MULTI_SHORT = 0x1000;	//  split long words to short words from left to right
    private $PSCWS4_MULTI_DUALITY = 0x2000;	//  split every long words(3 chars?) to two chars
    private $PSCWS4_MULTI_ZMAIN = 0x4000;	// split to main single chinese char atr = j|a|n?|v?
    private $PSCWS4_MULTI_ZALL = 0x8000;	// attr = ** , all split to single chars
    private $PSCWS4_MULTI_MASK = 0xf000;	// mask check for multi set
    private $PSCWS4_ZIS_USED = 0x8000000;

    /** single bytes segment flag (纯单字节字符) */
    private $PSCWS4_PFLAG_WITH_MB = 0x01;
    private $PSCWS4_PFLAG_ALNUM = 0x02;
    private $PSCWS4_PFLAG_VALID = 0x04;
    private $PSCWS4_PFLAG_DIGIT = 0x08;
    private $PSCWS4_PFLAG_ADDSYM = 0x10;
    /** constant var define */
	private $PSCWS4_WORD_FULL = 0x01;	// 多字: 整词
	private $PSCWS4_WORD_PART = 0x02;	// 多字: 前词段
	private $PSCWS4_WORD_USED = 0x04;	// 多字: 已使用
	private $PSCWS4_WORD_RULE = 0x08;	// 多字: 自动识别的
	private $PSCWS4_ZFLAG_PUT = 0x02;	// 单字: 已使用
	private $PSCWS4_ZFLAG_N2 = 0x04;	// 单字: 双字名词头
	private $PSCWS4_ZFLAG_NR2 = 0x08;	// 单字: 词头且为双字人名
	private $PSCWS4_ZFLAG_WHEAD = 0x10;	// 单字: 词头
	private $PSCWS4_ZFLAG_WPART = 0x20;	// 单字: 词尾或词中
	private $PSCWS4_ZFLAG_ENGLISH = 0x40;	// 单字: 夹在中间的英文
	private $PSCWS4_ZFLAG_SYMBOL = 0x80;	// 单字: 符号系列
	private $PSCWS4_MAX_ZLEN = 128;
	private $PSCWS4_MAX_EWLEN  = 16;
	...
	function __construct($charset = 'utf-8') {
        ...
        $this->set_dict(dirname(__FILE__) .'/scws/dict.utf8.xdb');
        $this->set_rule(dirname(__FILE__) .'/scws/rules.utf8.ini');
	}
	...
	{
		if ($yes == true) $this->_mode |= $this->PSCWS4_IGN_SYMBOL;
		else $this->_mode &= ~$this->PSCWS4_IGN_SYMBOL;
	}
	...
	{	
		...
		$this->_mode &= ~$this->PSCWS4_MULTI_MASK;
		if ($level & $this->PSCWS4_MULTI_MASK) $this->_mode |= $level;
	}

	// 设置是否显示分词调试信息
	function set_debug($yes)
	{
		if ($yes == true) $this->_mode |= $this->PSCWS4_DEBUG;
		else $this->_mode &= ~$this->PSCWS4_DEBUG;
	}

	// 设置是否自动将散字二元化
	function set_duality($yes)
	{
		if ($yes == true) $this->_mode |= $this->PSCWS4_DUALITY;
		else $this->_mode &= ~$this->PSCWS4_DUALITY;
	}
	...
	function get_result()
	{
		...
		$clen = $this->_ztab[$cx];
		$zlen = 1;
		$pflag = ($clen > 1 ? $this->PSCWS4_PFLAG_WITH_MB : ($this->_is_alnum($cx) ? $this->PSCWS4_PFLAG_ALNUM : 0));
		while (($off = ($off + $clen)) < $len)
		{
			...
			$clen = $this->_ztab[$cx];
			if (!($pflag & $this->PSCWS4_PFLAG_WITH_MB))
			{
				// pure single-byte -> multibyte (2bytes)
				if ($clen == 1)
				{
					if (($pflag & $this->PSCWS4_PFLAG_ALNUM) && !$this->_is_alnum($cx))
						$pflag ^= $this->PSCWS4_PFLAG_ALNUM;
				}
				else
				{
					if (!($pflag & $this->PSCWS4_PFLAG_ALNUM) || $zlen > 2) break;
					$pflag |= $this->PSCWS4_PFLAG_WITH_MB;
				}
			}
			else if (($pflag & $this->PSCWS4_PFLAG_WITH_MB) && $clen == 1)
			{
				// mb + single-byte. allowd: alpha+num + 中文
				if (!$this->_is_alnum($cx)) break;

				$pflag &= ~$this->PSCWS4_PFLAG_VALID;
				...
				if (!($pflag & $this->PSCWS4_PFLAG_VALID)) break;
				$clen += ($i - $off - 1);
			}
			// hightman.070813: add max zlen limit
			if (++$zlen >= $this->PSCWS4_MAX_ZLEN) break;
		}
		...
		else if ($pflag & $this->PSCWS4_PFLAG_WITH_MB) $this->_msegment($off, $zlen);
		else if (!($pflag & $this->PSCWS4_PFLAG_ALNUM) || (($off - $this->_off) >= $this->PSCWS4_MAX_EWLEN)) $this->_ssegment($off);
		else
		{
			$zlen = $off - $this->_off;
	...
	function get_tops($limit = 10, $xattr = '')
	{
		...
		while ($tmpa = $this->get_result())
		{
			foreach ($tmpa as $tmp)
			{
				...
				if ($this->_rule_checkbit($word, $this->PSCWS4_RULE_NOSTATS)) continue;
	...
	function _rule_load($fpath)
	{
		...
		while ($buf = fgets($fd, 512))
		{
			...
			if ($key == 'special') $item['bit'] = $this->PSCWS4_RULE_SPECIAL;
			else if ($key == 'nostats') $item['bit'] = $this->PSCWS4_RULE_NOSTATS;
			else 
			{
				$item['bit'] = (1<<$j);
				$j++;
			...
			if ($ch == ':')
			{
				...
				else if ($pkey == 'znum') 
				{
					if ($pos = strpos($pval, ','))
					{
						...
						$item['flag'] |= $this->PSCWS4_ZRULE_RANGE;
				...
				{
					if ($pval == 'prefix') $item['flag'] |= $this->PSCWS4_ZRULE_PREFIX;
					if ($pval == 'suffix') $item['flag'] |= $this->PSCWS4_ZRULE_SUFFIX;
				}
				else if ($pkey == 'include' || $pkey == 'exclude')
				{
					...
					{
						$item['inc'] |= $clude;
						$item['flag'] |= $this->PSCWS4_ZRULE_INCLUDE;
					}
					else
					{
						$item['exc'] |= $clude; 
						$item['flag'] |= $this->PSCWS4_ZRULE_EXCLUDE;
					}
				}
				continue;
			}
	...
	{
		if (($rule['flag'] & $this->PSCWS4_ZRULE_INCLUDE) && !$this->_rule_checkbit($str, $rule['bit']))
			return false;
		if (($rule['flag'] & $this->PSCWS4_ZRULE_EXCLUDE) && $this->_rule_checkbit($str, $rule['bit']))
			return false;
		return true;
	}
	...
	{
		return (($f & ($this->PSCWS4_ZFLAG_SYMBOL|$this->PSCWS4_ZFLAG_ENGLISH)) || (($f & ($this->PSCWS4_ZFLAG_WHEAD|$this->PSCWS4_ZFLAG_NR2)) == $this->PSCWS4_ZFLAG_WHEAD));
	}

	function _no_rule2($f)
	{
		//return (($f & $this->PSCWS4_ZFLAG_ENGLISH) || (($f & ($this->PSCWS4_ZFLAG_WHEAD|$this->PSCWS4_ZFLAG_N2)) == $this->PSCWS4_ZFLAG_WHEAD));
	...
	function _ssegment($end)
	{
		...
		if ($wlen > 1)
		{
			...
			if ($this->_rule_checkbit($txt, $this->PSCWS4_RULE_SPECIAL))
			{
		...
		while ($start < $end)
		{
			...
			{
				$pflag = $this->_is_digit($cx) ? $this->PSCWS4_PFLAG_DIGIT : 0;
				$wlen = 1;
				while ($start < $end)
				{
					$ch = $txt[$start];
					$cx = ord($ch);
					if ($pflag & $this->PSCWS4_PFLAG_DIGIT)
					{
						if (!$this->_is_digit($cx))
						{
							if (($pflag & $this->PSCWS4_PFLAG_ADDSYM) || $cx != 0x2e || !$this->_is_digit(ord($txt[$start+1])))
								break;
							$pflag |= $this->PSCWS4_PFLAG_ADDSYM;
						}
					}
					else
					{
						...
						{
							if (($pflag & $this->PSCWS4_PFLAG_ADDSYM) || $cx != 0x27 || !$this->_is_alpha(ord($txt[$start+1])))
								break;
							$pflag |= $this->PSCWS4_PFLAG_ADDSYM;
						}
					}
					$start++;					
					if (++$wlen >= $this->PSCWS4_MAX_EWLEN) break;
				}
				$this->_put_res($start - $wlen, 2.5*log($wlen), $wlen, 'en');
			}
			else if (!($this->_mode & $this->PSCWS4_IGN_SYMBOL))
			{
	...
	function _mget_word($i, $j)
	{
		...
		if (!($wmap[$i][$i]['flag'] & $this->PSCWS4_ZFLAG_WHEAD)) return $i;
		for ($r = $i, $k = $i+1; $k <= $j; $k++)
		{
			if ($wmap[$i][$k] && ($wmap[$i][$k]['flag'] & $this->PSCWS4_WORD_FULL)) $r = $k;
		}
		return $r;
	}
	...
	function _mset_word($i, $j)
	{
		...
		if (($item == false) || (($this->_mode & $this->PSCWS4_IGN_SYMBOL)
			&& !($item['flag'] & $this->PSCWS4_ZFLAG_ENGLISH) && $item['attr'] == 'un'))
		{
			return;
		}
		
		// hightman.070701: 散字自动二元聚合
		if ($this->_mode & $this->PSCWS4_DUALITY)
		{
			$k = $this->_zis;
			if ($i == $j && !($item['flag'] & $this->PSCWS4_ZFLAG_ENGLISH) && $item['attr'] == 'un')
			{
				$this->_zis = $i;
				if ($k < 0) return;
				
				$i = ($k & ~$this->PSCWS4_ZIS_USED);
				if (($i != ($j-1)) || (!($k & $this->PSCWS4_ZIS_USED) && $this->_wend == $i))
				{
					$this->_put_res($zmap[$i]['start'], $wmap[$i][$i]['idf'], $zmap[$i]['end'] - $zmap[$i]['start'], $wmap[$i][$i]['attr']);
					if ($i != ($j-1)) return;
				}
				$this->_zis |= $this->PSCWS4_ZIS_USED;
			}
			else
			{
				if (($k >= 0) && (!($k & $this->PSCWS4_ZIS_USED) || ($j > $i)))
				{
					$k &= ~$this->PSCWS4_ZIS_USED;
		...
		{
			$m = $i;
			if ($this->_mode & $this->PSCWS4_MULTI_SHORT)
			{
				while ($m < $j)
				{
					$k = $m;
					for ($n = $m + 1; $n <= $j; $n++)
					{
						if ($n == $j && $m == $i) break;
						$item = $wmap[$m][$n];
						if ($item && ($item['flag'] & $this->PSCWS4_WORD_FULL))
						{
							$k = $n;
							$this->_put_res($zmap[$m]['start'], $item['idf'], $zmap[$n]['end'] - $zmap[$m]['start'], $item['attr']);
							if (!($item['flag'] & $this->PSCWS4_WORD_PART)) break;
						}
					}
					if ($k == $m)
					{
			...
			if ($this->_mode & $this->PSCWS4_MULTI_DUALITY)
			{
				while ($m < $j)
				{
		...
		if (($j > $i) && ($this->_mode & ($this->PSCWS4_MULTI_ZMAIN|$this->PSCWS4_MULTI_ZALL)))
		{
			if (($j - $i) == 1 && !$wmap[$i][$j])
			{
				if ($wmap[$i][$i]['flag'] & $this->PSCWS4_ZFLAG_PUT) $i++;
				else $wmap[$i][$i]['flag'] |= $this->PSCWS4_ZFLAG_PUT;
				$wmap[$j][$j]['flag'] |= $this->PSCWS4_ZFLAG_PUT;
			}
			do
			{
				if ($wmap[$i][$i]['flag'] & $this->PSCWS4_ZFLAG_PUT) continue;
				if (!($this->_mode & $this->PSCWS4_MULTI_ZALL) && !strchr("jnv", substr($wmap[$i][$i]['attr'],0,1))) continue;
				$this->_put_res($zmap[$i]['start'], $wmap[$i][$i]['idf'], $zmap[$i]['end'] - $zmap[$i]['start'], $wmap[$i][$i]['attr']);
			}
			while (++$i <= $j);
		}
	...
	function _mseg_zone($f, $t)
	{
		...
		for ($x = $i = $f; $i <= $t; $i++)
		{
			...
			if ($j == $i || $j <= $x || (/* $i > $x && */($wmap[$i][$j]['flag'] & $this->PSCWS4_WORD_USED))) continue;
			...
			if ($i != $f && ($wmap[$i][$j]['flag'] & $this->PSCWS4_WORD_RULE)) continue;
			
			// create the new path 
			$wmap[$i][$j]['flag'] |= $this->PSCWS4_WORD_USED;
			...
			for ($m = $f; $m < $i; $m = $n+1)
			{
				...
				$npath[$x++] = $n - $m;
				if ($n > $m) $wmap[$m][$n]['flag'] |= $this->PSCWS4_WORD_USED;
			}
			...
			if ($this->_mode & $this->PSCWS4_DEBUG)
			{
	...
	function _msegment($end, $zlen)
	{
		...
		for ($i = 0; $start < $end; $i++)
		{
			...
			if ($clen == 1)
			{
				...
				$wmap[$i][$i] = array('tf'=>0.5, 'idf'=>0, 'flag'=> $this->PSCWS4_ZFLAG_ENGLISH, 'attr'=>'un');
			}
			else
			{
				...
				{					
					if (substr($query['attr'],0,1) == '#') $query['flag'] |= $this->PSCWS4_ZFLAG_SYMBOL;
					$wmap[$i][$i] = $query;
				}
				$start += $clen;
			}
		...
		for ($i = 0; $i < $zlen; $i++)
		{
			...
			for ($j = $i+1; $j < $zlen; $j++)
			{
				...
				$ch = $query['flag'];
				if ($ch & $this->PSCWS4_WORD_FULL)
				{
					$wmap[$i][$j] = $query;
					$wmap[$i][$i]['flag'] |= $this->PSCWS4_ZFLAG_WHEAD;
					
					for ($k = $i+1; $k <= $j; $k++) $wmap[$k][$k]['flag'] |= $this->PSCWS4_ZFLAG_WPART;
				}
				if (!($ch & $this->PSCWS4_WORD_PART)) break;
			}
			...
			if ($k--)
			{
				...
				if ($k == ($i+1))
				{
					if ($wmap[$i][$k]['attr'] == 'nr') $wmap[$i][$i]['flag'] |= $this->PSCWS4_ZFLAG_NR2;
					//if (substr($wmap[$i][$k]['attr'], 0, 1) == 'n') $wmap[$i][$i]['flag'] |= $this->PSCWS4_ZFLAG_N2;
				}
				
				// clean the PART flag for the last word
				if ($k < $j) $wmap[$i][$k]['flag'] ^= $this->PSCWS4_WORD_PART;
			}
		}
		...
		if (count($this->_rd) > 0)
		{
			...
			for ($i = 0; $i < $zlen; $i++)
			{
				...
				if (($r1['flag'] & $this->PSCWS4_ZRULE_PREFIX) && ($i < ($zlen - $clen)))
				{
					...
					// 注意原来2字人名,识别后仍为2字的情况
					if ($wmap[$i][$i]['flag'] & $this->PSCWS4_ZFLAG_NR2)
					{
						if ($clen == 1) continue;
						$wmap[$i][$i+1]['flag'] |= $this->PSCWS4_WORD_PART;
					}

					// ok, got: i & clen 
					$k = $i + $clen;
					$wmap[$i][$k] = array('tf'=>$r1['tf'], 'idf'=>$r1['idf'], 'flag'=>($this->PSCWS4_WORD_RULE|$this->PSCWS4_WORD_FULL), 'attr'=>$r1['attr']);
					$wmap[$i][$i]['flag'] |= $this->PSCWS4_ZFLAG_WHEAD;
					for ($j = $i+1; $j <= $k; $j++) $wmap[$j][$j]['flag'] |= $this->PSCWS4_ZFLAG_WPART;
					
					if (!($wmap[$i][$i]['flag'] & $this->PSCWS4_ZFLAG_WPART)) $i = $k;
					continue;
				}
				
				if (($r1['flag'] & $this->PSCWS4_ZRULE_SUFFIX) && ($i >= $clen))
				{
					// suffix, check before
					for ($ch = 1; $ch <= $clen; $ch++)
					{
					...
					$wmap[$k][$i] = array('tf'=>$r1['tf'], 'idf'=>$r1['idf'], 'flag'=>$this->PSCWS4_WORD_FULL, 'attr'=>$r1['attr']);
					$wmap[$k][$k]['flag'] |= $this->PSCWS4_ZFLAG_WHEAD;
					for ($j = $k+1; $j <= $i; $j++)
					{
						$wmap[$j][$j]['flag'] |= $this->PSCWS4_ZFLAG_WPART;
						if (($j != $i) && ($wmap[$k][$j] != false)) $wmap[$k][$j]['flag'] |= $this->PSCWS4_WORD_PART;
					}
					continue;
				}
			}
			...
			for ($i = $zlen - 2; $i >= 0; $i--)
			{
				...
				if (($wmap[$i][$i+1] == false) || ($wmap[$i][$i+1]['flag'] & $this->PSCWS4_WORD_PART)) continue;
				...
				$clen = $r1['zmin'] > 0 ? $r1['zmin'] : 1;
				if (($r1['flag'] & $this->PSCWS4_ZRULE_PREFIX) && ($k < ($zlen - $clen)))
				{
					for ($ch = 1; $ch <= $clen; $ch++)
					{
						$j = $k + $ch;
					...
					// ok, got: i & clen
					$k = $k + $clen;
					$wmap[$i][$k] = array('tf'=>$r1['tf'], 'idf'=>$r1['idf'], 'flag'=>$this->PSCWS4_WORD_FULL, 'attr'=>$r1['attr']);
					$wmap[$i][$i+1]['flag'] |= $this->PSCWS4_WORD_PART;
					for ($j = $i+2; $j <= $k; $j++) $wmap[$j][$j]['flag'] |= $this->PSCWS4_ZFLAG_WPART;
					$i--;
					continue;
				}
				...
				if (($r1['flag'] & $this->PSCWS4_ZRULE_SUFFIX) && ($i >= $clen))
				{
					...
					{
						$wmap[$j][$j]['flag'] |= $this->PSCWS4_ZFLAG_WPART;
						if ($wmap[$k][$j] != false) $wmap[$k][$j]['flag'] |= $this->PSCWS4_WORD_PART;
					}
					$i -= ($clen+1);
					continue;
				}
		...
		{
			if ($wmap[$i][$i]['flag'] & $this->PSCWS4_ZFLAG_WPART) continue;
			if ($i > $j) $this->_mseg_zone($j, $i-1);

			$j = $i;
			if (!($wmap[$i][$i]['flag'] & $this->PSCWS4_ZFLAG_WHEAD))
			{
				$this->_mset_word($i, $i);
				$j++;
			}
		...
		if (($this->_mode & $this->PSCWS4_DUALITY) && ($this->_zis >= 0) && !($this->_zis & $this->PSCWS4_ZIS_USED))
		{
			$i = $this->_zis;
			$this->_put_res($zmap[$i]['start'], $wmap[$i][$i]['idf'], $zmap[$i]['end'] - $zmap[$i]['start'], $wmap[$i][$i]['attr']);	
		}
