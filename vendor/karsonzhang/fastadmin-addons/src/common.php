...
function get_package_addon(&$group = array()){
    $Addons = new \fast\Addon();
	$results = scandir($Addons::getAddonPathUrl());
    $list = [];
    foreach ($results as $name)
    {
        if ($name === '.' or $name === '..') continue;
        $addonDir = $Addons::getAddonPathUrl() . DS . $name . DS;
        if (!is_dir($addonDir)) continue;

		$install = $addonDir . 'install/Install.php';
        if (!is_file($install)) continue;

        //这里不采用get_addon_info是因为会有缓存
        //$info = get_addon_info($name);
        $info_file = $addonDir . 'install/info.ini';
        if (!is_file($info_file)) continue;
        $info = Config::parse($info_file, '', "addon-info-{$name}");
		$addonInfo = $Addons::getAddonByPath($name);
        $install = ($addonInfo) ? 1 : 0;
		$info['install'] = $install ? 1 : 0;
		$info['status'] = $install && $addonInfo['status'] ? 1 : 0;
		$info['config'] = isset($info['config']) ? $info['config'] : 0;
        $list[$name] = $info;
    }
    return $list;
}
...
function get_addon_autoload_config($truncate = false)
{
    ...
    $base = array_merge($base, ['install', 'uninstall', 'enable', 'disable', 'importsql']);
...
{
    $addonPath = $Addons::getAddonPathUrl();
    $file = $addonPath . DS . $name . DIRECTORY_SEPARATOR . 'info.ini';
...
{
    $file = \fast\Addon::getAddonPathUrl() . DS . $name . DS . 'config.php';
    if(!file_exists($file)) {
...
//        throw new Exception("文件不存在:". $file);
    ...
    {
        throw new Exception("文件没有写入权限:". $file);
    }
    if ($handle = fopen($file, 'w'))
    {
