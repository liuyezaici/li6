...
class Tree
{
     ...
     *      )
     */
    public function init($arr = [], $pidname = NULL, $nbsp = NULL, $fix = false)
    {
        $this->arr = $arr;
        if (!is_null($pidname))
	...
	*/
		...
		foreach($this->arr as &$v){
			...
			if($v[$this->pidname]){
				foreach($this->arr as $vv){
					if($vv['id'] == $v[$this->pidname])$flag = true;
				}
				if(!$flag)$v[$this->pidname] = 0;
			}
		}
		unset($v);
	}
     ...
     * 菜单数据
     * @param int $myid
     * @param string $itemtpl '<li class="@class"><a href="@url@addtabs" addtabs="@id" url="@url" py="@py" pinyin="@pinyin"><i class="@icon"></i> <span>@title</span> <span class="pull-right-container">@caret @badge</span></a> @childlist</li>'
     ...
     * @return string
     */
    public function getTreeMenu($myid, $itemtpl='<li class="@class"><a href="@url@addtabs" addtabs="@id" url="@url"><i class="@icon"></i> <span>@title</span> <span class="pull-right-container">@caret @badge</span></a> @childlist</li>', $selectedids = '', $disabledids = '', $wraptag = 'ul', $wrapattr = '', $deeplevel = 0)
    {
        $str = '';
        $childs = $this->getChild($myid);
        if ($childs)
