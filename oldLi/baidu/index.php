            ...
            if ($filejson !== 1) {
                ...
                if ($filejson["errno"] != 0) echo '<div class="row justify-content-center"><div class="col-md-7 col-sm-8 col-11"><div class="alert alert-danger" role="alert">
                    ...
                    for ($i = 0; $i < count($filejson["list"]); $i++) {
                        ...
                        print_r('$file["fs_id"]');
                        print_r($file["fs_id"].PHP_EOL);
                        print_r('$timestamp');
                        print_r($timestamp.PHP_EOL);
                        print_r('$sign');
                        print_r($sign.PHP_EOL);
                        print_r('$randsk');
                        print_r($randsk.PHP_EOL);
                        print_r('$shareid');
                        print_r($shareid.PHP_EOL);
                        print_r('$uk');
                        print_r($uk.PHP_EOL);
                        $json4 = getDlink($file["fs_id"], $timestamp, $sign, $randsk, $shareid, $uk);
                        if ($json4["errno"] == 0) {
                            $item = $json4["list"][0];
                            $dlink = $item["dlink"];
                            $server_filename = $item["server_filename"];
                            $array_ = explode('.', $server_filename);
                            $ext = '.'.end($array_);
                            //尾部组合格式 才能下载
                            //https://d.pcs.baidu.com/file/13e1da57e670d01725c9282ec75ab03a?fid=320971386-250528-366627758249232&dstime=1598364094&rt=sh&sign=FDtAERVYyK-DCb740ccc5511e5e8fedcff06b081203-%2FCnMhpsVY3PK%2BG5iJYzOjHOS4%2F0%3D&expires=8h&chkv=1&chkbd=0&chkpc=et&dp-logid=8976748989836077965&dp-callid=0&shareid=8102380&r=965518870
                            //&clienttype=0&vuk=9223372036854493472&vip=0&ext=.mp3
                            $dlink .= '&clienttype=0&vip=0&ext='.$ext;
//                        print_r('$dlink');
//                        print_r($dlink);
