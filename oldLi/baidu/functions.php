...
function getSign(string $surl, $randsk) {
	...
	$res =  get($url, $header);
	if (preg_match('/yunData.setData\((\{.*?\})\);/', $res, $matches)) {
//	    print_r($matches[1]);
...
function getDlink(string $fs_id, string $timestamp, string $sign, string $randsk, string $share_id, string $uk) { // 获取下载链接
    ...
    //sign=b2831c8510c36f66438ba083c7b7005c0352ef62
    //&timestamp=1598359327
    //&channel=chunlei
    //&web=1
    //&app_id=250528
    //&bdstoken=null
    //&logid=NDdCRkRBNzMxNzQ0Rjc0MkE2MUNENzJBMjQ0MEQxQ0M6Rkc9MQ==
    //&clienttype=0
    //
