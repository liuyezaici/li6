...
namespace Func;
use \Exception;
if(!class_exists('JsonProtocol2')) create_JsonProtocol2();
...
class RpcClientWallet
{
    ...
    {
        $this->openConnection();
        $this->bin_data = JsonProtocol2::encode(array(
                'class'              => $this->serviceName,
                'method'         => $method,
                'param_array'  => $arguments,
                ));
        if(fwrite($this->connection, $this->bin_data) !== strlen($this->bin_data))
        {
     ...
     * @param array $arguments
     */
    public function sendCodes($msgId, $cmd, $string)
    {
        $this->openConnection();
        $this->bin_data = JsonProtocol2::encode(array(
            'msgId'         => $msgId,
    ...
    public function recvData()
    {
        ...
        if(!$ret) return '';
        return JsonProtocol2::decode($ret);
    }
