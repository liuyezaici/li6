...
class drag_validate
{
    private $sessionTag = 'lr_validate_pic';
    private $wrongSessionName = 'lr_validate_pic_wrong_time';

    private $tailoring_big_save_path;    //拼图保存路径
    private $tailoring_small_save_path;  //小截图保存路径
    private $session_name = '';    //Session Name
    ...
    private $pic_info;
    private $clear_cahce_time= 300; //自动清理缓存的图片时间 默认300秒

    public function __construct()
    {
        @session_start();
        $this->init();
        $this->session_name = $this->sessionTag; //  设置session下标
    ...
    //校验验证码相差多少
    public function compareValidate($enterCode){
        return bcsub(floatval($enterCode), isset($_SESSION[$this->sessionTag]) ? $_SESSION[$this->sessionTag]: 0, 2);
    }
    //获取错误次数
    public function getWrongTimes() {
        return isset($_SESSION[$this->wrongSessionName])?$_SESSION[$this->wrongSessionName]: 0;
    }
    //设置错误次数
    public function setWrongTimes($newTimes) {
        $_SESSION[$this->wrongSessionName] = $newTimes;
    }
    //清空验证码和错误次数
    public function clearAllCache(){
        $this->clearCode();
        $this->clearWrongCache();
    }
    //清空错误次数
    public function clearWrongCache(){
        $_SESSION [$this->wrongSessionName] = '';
    }
    //清空验证码
    public function clearCode(){
        $_SESSION [$this->sessionTag] = '';
    }
    /**
     * @name 初始化验证码数据
     */
    public function init()
    {
        ...
        $this->ResourcesPath =  root.'include/lib/drag_validate/img/'; //背景图片存放目录 （Resources）
        $dir = 'cache/img';  //展示图片缓存目录(相对当前类php文件路径)（裁剪小图，错位背景图,随时可能被删除！所以必须确认此目录是可以移除的）
        ...
        //print_r($this->PicSavePath);
        //exit;
        $this->_clearFile($this->PicSavePath, time()-$this->clear_cahce_time);//清理1小时以前的图片
    }
    ...
    private function getRandomPng() {
        ...
        if(!file_exists($this->srcPic )) {
            throw new \Exception('图片资源'. $this->srcPic .'不存在！');
        } else {
            return  $this->srcPic;
        }
    }
     ...
     * @name 删除一小时之前的 验证码图片
     */
    public function _clearFile($sPath, $iTimeLine){
        if(is_dir($sPath)){
            $fp = opendir($sPath);
            while(!false == ($fn = readdir($fp))){
                if($fn == '.' || $fn =='..') continue;
                $sFilePath = $sPath.'/'.$fn;
                $this->_clearFile($sFilePath ,$iTimeLine);
            }
        }else{
            if($sPath != '.' && $sPath != '..'){
                //. ..文件直接跳过，不处理
    ...
    public function RestructuringCutting($srcFile,$bigPicName,$thumbnailWide,$thumbnailHeight)
    {
        ...
        $_SESSION[$this->session_name] = $this->location_x;
        return array(
