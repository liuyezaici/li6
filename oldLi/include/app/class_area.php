...
class area
{
    ...
    {
        $db = mysql::getInstance();
        $provinceInfo = DbBase::getRowBy("s_prov_city_area_street", "s_name", "s_id = '". $id_ ."' limit 0,1");
...
//        print_r($allArea);exit;
    ...
    //递归获取所有父权限
    public static function getAllAreaDigui() {
        $db = mysql::getInstance();
        $allArea = $db->getAll('s_prov_city_area_street', 's_id,s_name,s_parent_id', "1");
        $allArea = Str::diguiArray($allArea, 0, 'sons', 's_parent_id', 's_id');
        return $allArea;
    }
