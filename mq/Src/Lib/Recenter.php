...
class Recenter
{
    ...
    {
        self::$streamName = \CommonCfg::get('mqRules.recenterName');
...
//                print_r($datum);
    ...
    private static function _insertData($msgId, $datumString, $customer='')
    {
        try {
            ...
            if($tmpData) {
                $status = MsgTask::dealRecenter($datumString);
                //保证其他人正常 先屏蔽错误
                if($status !== true) {
                    throw new \Exception("insertRecenterData_error:".$status);
                }
            }
            //执行完成 删除stream的msgid
